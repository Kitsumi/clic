...
function commandHandler(text) {
    ...
    if (command == "killserver") {
        process.exit();
    } if (command == "databasereload") {
        jsonfile.readFile(__dirname+"/database.json", function(err, obj) {
            if (err == null) {
                database = obj;
                write('* {green-fg}Database reloaded successfully{/} *');
             } else {
                console.log("Database reload error: "+err);
             }
        });
    } else {
...
io.on('connection', function(socket){
    ...
    if (!(/\A\z/.test(pass))) {
        if (socket.nick.toLowerCase() != "anon") {
        ...
        socket.emit('server-message', '{red-fg}You can\'t have a blank password!{/}');
    }
    });
